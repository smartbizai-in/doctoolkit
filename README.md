...
`resources/icon-source.png` (1254×1254, background already baked in) is the real app icon
source as of 2026-08-31 — `capacitor-assets generate` against the old `resources/logo.svg`
produced a transparent legacy icon instead of compositing the intended background, so the
Android mipmap files (legacy `ic_launcher`/`ic_launcher_round` + adaptive
`ic_launcher_foreground`/`ic_launcher_background`, all 6 densities) are generated by hand from
`icon-source.png` instead — see the git history for the exact script. `resources/logo.svg` is
kept only as the earlier vector placeholder; it's no longer what ships.
